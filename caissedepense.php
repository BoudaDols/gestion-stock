...
$pagestitle = "Mise à jour des dépenses";
...
//Pagination
$parpage = 10;
$countResult = SQLSelect("SELECT * FROM reglement WHERE statutReglement = 'D' AND reglement_codeFacture = ''");
...
if (isset($_POST['btnresearch'])) {
    $rech = $_POST['research'];
    if ($rech == "") {
        $sqlrech = "SELECT * FROM reglement WHERE statutReglement = 'D' AND reglement_codeFacture = '' LIMIT :offset, :limit";
        $sqlrechParams = [':offset' => $first, ':limit' => $parpage];
        $tableau = "entier";
    } else {
        $sqlrech = "SELECT * FROM reglement WHERE statutReglement = 'D' AND reglement_codeFacture = '' AND objetReglement LIKE :rech LIMIT :offset, :limit";
...
?>
	...
	<?php
		if ($tableau == "entier") {
			$entrees = SQLSelect("SELECT * FROM reglement WHERE statutReglement = 'D' AND 
